...
describe('<ScrumAssistantChatHeader />', () => {
  ...
  it('se monta correctamente con todos los elementos principales', () => {
    render(<MeetAssistantChatHeader />);

    expect(screen.getByTestId('chat-header')).toBeInTheDocument();
    expect(screen.getByTestId('header-title')).toHaveTextContent('Asistente de reuniones/ceremonias Scrum');
  ...
  describe('Diálogo de ayuda', () => {
    ...
    it('abre el diálogo al hacer clic en el botón de ayuda', async () => {
      ...
      expect(screen.getByTestId('dialog-title')).toHaveTextContent('Guía del Asistente de reuniones/ceremonias Scrum');
    });
  ...
  it('cumple con estándares básicos de accesibilidad', async () => {
    ...
    const mainHeading = await screen.findByRole('heading', {
      name: 'Guía del Asistente de reuniones/ceremonias Scrum',
      level: 2,
    });
